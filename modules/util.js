...
var copyFiles = function (srcRoot, src, dst) {
    if (!fs.existsSync(src))
        return;

    var targetPath = fs.readdirSync(src);

    for (var i = 0; i < targetPath.length; i++) {
        var filePath = path.join(src, targetPath[i]);
        var fileStat = fs.lstatSync(filePath);

        if (fileStat.isDirectory())
            copyFiles(srcRoot, filePath, dst);
        else {
            var newPath = filePath.replace(srcRoot, dst);

            if (!fs.existsSync(path.dirname(newPath)))
                mkdirp.sync(path.dirname(newPath));

            console.log("[copy] %s -> %s", filePath, newPath);
